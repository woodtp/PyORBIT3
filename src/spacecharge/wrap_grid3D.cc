...
static int Grid3D_init(pyORBIT_Object *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTuple(args, "iii:__init__", &binX, &binY, &binZ)) {
    ORBIT_MPI_Finalize(
        "PyGrid3D - Grid3D(nX,nY,nZ) - constructor needs parameters.");
  }
...
static PyObject *Grid3D_setValue(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "diii:setValue", &val, &ix, &iy, &iz)) {
    ORBIT_MPI_Finalize(
        "PyGrid3D - setValue(val,ix,iy,iz) - parameters are needed.");
  }
...
static PyObject *Grid3D_getValueOnGrid(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "iii:getValueOnGrid", &ix, &iy, &iz)) {
    ORBIT_MPI_Finalize(
        "PyGrid3D - getValueOnGrid(ix,iy,iz) - parameters are needed.");
  }
...
static PyObject *Grid3D_getGridX(PyObject *self, PyObject *args) {
  ...
  Grid3D *cpp_Grid3D = (Grid3D *)pyGrid3D->cpp_obj;
  int ind = -1;
  if (!PyArg_ParseTuple(args, "i:getGridX", &ind) || ind < 0 ||
      ind >= cpp_Grid3D->getSizeX()) {
    ORBIT_MPI_Finalize(
        "PyGrid3D - getGridX(ix) - parameter is needed. [0 - sizeX[");
  }
...
static PyObject *Grid3D_getGridY(PyObject *self, PyObject *args) {
  ...
  Grid3D *cpp_Grid3D = (Grid3D *)pyGrid3D->cpp_obj;
  int ind = -1;
  if (!PyArg_ParseTuple(args, "i:getGridY", &ind) || ind < 0 ||
      ind >= cpp_Grid3D->getSizeY()) {
    ORBIT_MPI_Finalize(
        "PyGrid3D - getGridY(iy) - parameter is needed. [0 - sizeY[");
  }
...
static PyObject *Grid3D_getGridZ(PyObject *self, PyObject *args) {
  ...
  Grid3D *cpp_Grid3D = (Grid3D *)pyGrid3D->cpp_obj;
  int ind = -1;
  if (!PyArg_ParseTuple(args, "i:getGridZ", &ind) || ind < 0 ||
      ind >= cpp_Grid3D->getSizeZ()) {
    ORBIT_MPI_Finalize(
        "PyGrid3D - getGridZ(iz) - parameter is needed. [0 - sizeZ[");
  }
...
static PyObject *Grid3D_synchronizeMPI(PyObject *self, PyObject *args) {
  ...
  if (nVars == 0) {
    ...
    PyObject *py_mpi_comm_type =
        wrap_orbit_mpi_comm::getMPI_CommType("MPI_Comm");
    PyObject *pyMPIComm = PyTuple_GetItem(args, 0);
    if ((!PyObject_IsInstance(pyMPIComm, py_mpi_comm_type))) {
      ORBIT_MPI_Finalize("Grid3D.synchronizeMPI(MPI_Comm) - input "
                         "parameter is not MPI_Comm");
    }
...
static PyObject *Grid3D_longWrapping(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "|O:longWrapping", &pyIsWrapped)) {
    ORBIT_MPI_Finalize("PyGrid3D - longWrapping([True/False]) - "
...
static PyObject *Grid3D_binBunch(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O|d:binBunch", &pyBunch, &lambda)) {
    ORBIT_MPI_Finalize("PyGrid3D - binBunch(Bunch* bunch [,lambda]) - "
                       "parameters are needed.");
  }
  PyObject *pyORBIT_Bunch_Type = wrap_orbit_bunch::getBunchType("Bunch");
  if (!PyObject_IsInstance(pyBunch, pyORBIT_Bunch_Type)) {
    ORBIT_MPI_Finalize("PyGrid3D - binBunch(Bunch* bunch [,lambda]) - "
                       "method needs a Bunch.");
  }
...
static PyObject *Grid3D_binValue(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "dddd:binValue", &val, &x, &y, &z)) {
    ORBIT_MPI_Finalize(
        "PyGrid3D - binValue(val,x,y,z) - parameters are needed.");
  }
...
static PyObject *Grid3D_calcGradient(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "ddd:calcGradient", &x, &y, &z)) {
    ORBIT_MPI_Finalize(
        "PyGrid3D - calcGradient(x,y,z) - parameters are needed.");
  }
...
static PyObject *Grid3D_from_numpy(PyObject *self, PyObject *args) {
  ...
  PyArrayObject *arr = (PyArrayObject *)PyArray_FROM_OTF(arr_in, NPY_FLOAT64,
                                                         NPY_ARRAY_IN_ARRAY);
  if (NULL == arr) {
    return NULL;
  }
  ...
  if (PyArray_NDIM(arr) != 3) {
    ...
    PyErr_SetString(
        PyExc_ValueError,
        "from_numpy: array must be 3-dimensional with shape (nz,nx,ny)");
    return NULL;
  }
  ...
  if (nz_in != nz_grid || nx_in != nx_grid || ny_in != ny_grid) {
    ...
    PyErr_SetString(
        PyExc_ValueError,
        "from_numpy: shape mismatch; expected (zSize, xSize, ySize)");
    return NULL;
  }
...
static PyTypeObject pyORBIT_Grid3D_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "Grid3D",  /*tp_name*/
