...
extern "C" {
  ...
  static void Collimator_del(pyORBIT_Object* self){
   ...
   {NULL}
    ...
    //--------------------------------------------------
    static struct PyModuleDef cModPyColl =
    {
        PyModuleDef_HEAD_INIT,
        "collimator", "Collimator classes collection",
        -1,
        CollimatorClassMethods
    };
    PyMODINIT_FUNC initcollimator(){
        //check that the Collimator wrapper is ready
        if (PyType_Ready(&pyORBIT_Collimator_Type) < 0)
         return NULL;
